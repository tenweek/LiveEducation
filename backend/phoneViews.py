...
def templateSMS(accountSid,accountToken,appId,toNumbers,templateId,param,isUseJson):
	...
	url = "https://api.ucpaas.com" + ":" + "" + "/" + "2014-06-30" + "/Accounts/" + accountSid + "/Messages/templateSMS?sig=" + signature
	if isUseJson == True:
...
def phoneTest(request):
	...
	print(templateSMS(accountSid,accountToken,appId,toNumber,templateId,param,True))
	response = JsonResponse({'verification': param})
	return response

@csrf_exempt
def phoneRand(request):
	req = simplejson.load(request)
	user = User.objects.filter(username=req['phoneNum'])
	if len(user) == 0:
